...
, mComboLabel("Target dictionary: ")
{
    ...
    mTranslationInput.signal_changed().connect([this](){
        this->check_validity();
    });
    ...
    mAddButton.signal_clicked().connect([this]() {
        ...
        {
            if(dict->addWord(mWordInput.get_text(),
                             mTranslationInput.get_text()))
            {
                this->hide();
            }
        }
    ...
    //create model for combobox with Dict selection
    {
        ...
        if((mCombobox.get_active_row_number() == -1) && (mLogic.mDicts.size() > 0))
...
void NewWordWindow::check_validity()
{
    ...
    if(std::any_of(t.begin(), t.end(), [](auto &p) { return std::isspace(p); }))
    {
    ...
    if(std::all_of(t2.begin(), t2.end(),
                       [](auto &p) { return std::isspace(p); }))
    {
